...
/**
 * @title CryticMultiSigNonceProperties
 * @notice Nonce management properties for MultiSig wallets
 ...
 */
abstract contract CryticMultiSigNonceProperties is CryticMultiSigBase {
    ...
    /**
     * @notice MULTISIG-NONCE-001: Nonce is strictly monotonically increasing
     ...
     */
    function test_MultiSig_nonceMonotonicallyIncreases() public {
    ...
    /**
     * @notice MULTISIG-NONCE-002: Each nonce can only be used once
     * @dev Invariant: Once a nonce is used (marked as used), it should stay marked
     *      This directly addresses the requirement from issue #32
     * @param nonce Nonce value to check
     */
    function test_MultiSig_nonceUsedOnlyOnce(uint256 nonce) public {
        uint256 currentNonce = getNonce();
    ...
    /**
     * @notice MULTISIG-NONCE-003: Nonce only increments on successful execution
     * @dev Invariant: Failed transactions or pending transactions should not increment nonce
     * @param txHash Transaction identifier
     */
    function test_MultiSig_nonceIncrementsOnlyOnExecution(bytes32 txHash) public {
        uint256 currentNonce = getNonce();
    ...
    /**
     * @notice MULTISIG-NONCE-004: Old nonces cannot be reused
     ...
     */
    function test_MultiSig_oldNoncesRejected(uint256 txNonce) public {
        uint256 currentNonce = getNonce();
    ...
    /**
     * @notice MULTISIG-NONCE-005: Nonce starts at expected initial value
     * @dev Invariant: Initial nonce should remain constant
     *      Most implementations start at 0 or 1
     */
    function test_MultiSig_nonceStartsAtExpectedValue() public {
        uint256 currentNonce = getNonce();
    ...
    /**
     * @notice MULTISIG-NONCE-006: No gaps in nonce sequence
     ...
     */
    function test_MultiSig_noNonceGaps(uint256 nonce) public {
        uint256 currentNonce = getNonce();
