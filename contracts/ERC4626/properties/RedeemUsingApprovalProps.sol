...
contract CryticERC4626RedeemUsingApproval is CryticERC4626PropertyBase, CryticERC4626VaultProxy {
    ...
    function verify_redeemViaApprovalProxy(uint256 receiverId, uint256 shares) public returns (uint256 tokensWithdrawn){
    ...
    function verify_withdrawViaApprovalProxy(uint256 receiverId, uint256 tokens) public returns (uint256 sharesBurned){
        address owner = address(this);
        address receiver = restrictAddressToThirdParties(receiverId);
    ...
    function verify_withdrawRequiresTokenApproval(uint256 receiverId, uint256 tokens, uint256 sharesApproved) public {
        ...
        try redemptionProxy.withdrawOnBehalf(tokens, receiver, owner) returns (uint256 _sharesBurned) {
            assertLte(_sharesBurned, sharesApproved, "Redemption proxy must not be able to withdraw more shares than it was approved");
        } catch {  }
    }
    ...
    function verify_redeemRequiresTokenApproval(uint256 receiverId, uint256 shares, uint256 sharesApproved) public {
        ...
        try redemptionProxy.redeemOnBehalf(shares, receiver, owner) returns (uint256 _sharesBurned) {
            assertLte(_sharesBurned, sharesApproved, "Redemption proxy must not be able to redeem more shares than it was approved");
        } catch {  }
    }
}
