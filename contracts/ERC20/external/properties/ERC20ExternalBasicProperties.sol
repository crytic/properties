...
abstract contract CryticERC20ExternalBasicProperties is CryticERC20ExternalTestBase {
    ...
    function test_ERC20external_transferFromMoreThanBalance(address target) public {
        ...
        assertEq(token.balanceOf(msg.sender), balance_sender, "TransferFrom for more than balance modified source balance");
        assertEq(token.balanceOf(target), balance_receiver, "TransferFrom for more than balance modified target balance");
    }
    ...
    function test_ERC20external_transferMoreThanBalance(address target) public {
        ...
        assertEq(token.balanceOf(address(this)), balance_sender, "Transfer for more than balance modified source balance");
    ...
    function test_ERC20external_transferFrom(address target, uint256 amount) public {
        ...
        assertWithMsg(r == true, "transfer failed");
        assertEq(token.balanceOf(msg.sender), balance_sender - transfer_value, "Wrong source balance after transferFrom");
    ...
    function test_ERC20external_spendAllowanceAfterTransfer(address target, uint256 amount) public {
        ...
        if (current_allowance != type(uint256).max) {
            assertEq(token.allowance(msg.sender, address(this)), current_allowance - transfer_value, "Allowance not updated correctly");
        }
    }
